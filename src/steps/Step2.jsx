...
function mapSourcesToStepComponents(...args) {
    const datasources = [];
    ...
    return extractConfigKeys.reduce((acc, k) => {
        return {
            ...acc,
            [k]: {
                ...window[window.sessionStorage?.tabId].dp
                    .load(source)
                    .extractData({
...
const fp = (price) => {
    ...
    return `${parsedPrice}`.includes('.')
        ? parsedPrice.toFixed(2)
        : parsedPrice;
};

const chargesTransform = [
    (v) => {
        if (v != null) {
            const amount = fp(v);
...
const mappingConfig = {
    ...
    planDetails: [
            ...
            transformations: [
                (priceSummary, linePriceDetails) => {
                    ...
                    linePriceDetails.forEach((lpd) => {
                        const { fees = [], govt = [] } = (
                            lpd?.taxComponents?.serviceTaxInfo || []
                        ).reduce(
                            (acc, { serviceTaxList = [] }) => {
                        ...
                        const hotlineSuspended =
                            lpd?.telephoneData?.statusActvCode === 'SUS' &&
                            lpd?.telephoneData?.statusActvRsnCode === 'CO';

                        summaries.fees.push(fees);
                        summaries.govt.push(govt);
                        summaries.current.plan.push(
                            lpd?.currentCharges?.currentRatePlan || {}
                        );
                        summaries.current.addOn.push(
                            lpd?.currentCharges?.currentAddOns || []
                        );
                        summaries.proposed.plan.push(
                            hotlineSuspended
                                ? {}
                                : lpd.currentPlanPricingInfo || {}
                        );
                        summaries.proposed.addOn.push(
                            hotlineSuspended
                                ? []
                                : lpd.currentAddOnPricingInfo || []
                        );
                        summaries.nextMonth.plan.push(
                            lpd.nextBillPlanPricingInfo || []
                        );
                        summaries.nextMonth.addOn.push(
                            lpd.nextBillAddOnPricingInfo || []
                        );
                    });
                ...
                (linePriceSummaries) => {
                    ...
                    proposed.plan.forEach((cp) => {
                        res.proposed.totalPlanCharges +=
                            (cp.dueAmount || 0) - (cp?.adjustmentAmount || 0);
                    });
                    proposed.addOn.forEach((ca) => {
                        ca.forEach((addon) => {
                            res.proposed.totalFeatureCharges +=
                                addon.dueAmount ||
                                0 - addon?.adjustmentAmount ||
                                0;
                        });
                    });
                    ...
                    nextMonth.addOn.forEach((ca) => {
                        ca.forEach((addon) => {
                            res.nextMonth.totalFeatureCharges +=
                                addon.dueAmount || 0;
                        });
                    });
                    linePrice.forEach((lps) => {
                        res.proposed.total +=
                            (lps.dueAmount || 0) - (lps?.adjustmentAmount || 0);
                        res.nextMonth.total += lps.dueAmountNextMonth;
                    });

                    res.current.total =
                        res.current.totalFeatureCharges +
                        res.current.totalPlanCharges;
        ...
        },
        {
            ...
            transformations: [
                (linePriceDetails) => {
                    const result = linePriceDetails.map(
                        ({
                            ctn = '',
                            telephoneData = '',
                            imei = '',
                            ...
                            const { serviceTaxInfo = [] } = taxComponents;
                            const {
                                fees = [],
                                govt = [],
                            } = serviceTaxInfo.reduce(
                                (acc, { serviceTaxList = [] }) => {
                                    serviceTaxList.forEach((taxItem) => {
                                        if (
                                            taxItem.taxCategory
                ...
                (priceDetailsByCTN) => {
                    ...
                    priceDetailsByCTN.forEach(
                        ({
                            ...
                            const lineTotalPrice = current?.latestCharge?.reduce(
                                function (tot, arr) {
                            ...
                            const mappedCharges = {
                                ...
                                proposed: {
                                    planCharges: {
                                        ...
                                        total: hotlineSuspended
                                            ? 0
                                            : now?.plan?.dueAmount -
                                                  now?.plan?.adjustmentAmount ||
                                              0,
                                        meta:
                                            Object.keys(now?.plan).length === 0
                                                ? next?.plan
                                                : now?.plan,
                                        showProration:
                                            now?.ctnStatus !== undefined &&
                                            !hotlineSuspended,
                                    },
                                ...
                                nextMonth: {
                                    planCharges: {
                                        discount:
                                            linePrice?.totalDiscountNextMonth ||
                                            0,
                                        total:
                                            next?.plan?.dueAmount -
                                                next?.plan?.adjustmentAmount ||
                                            0,
                                        meta: next?.plan || {},
                                    },
                                    ...
                                    govtCharges,
                                    lineTotal:
                                        linePrice?.dueAmountNextMonth || 0,
                                },
                            };

                            [{ bill: current, name: 'current' }].forEach(
                                ({ bill, name }) => {
...
export default function Step2(props) {
    ...
    let { getplansandaddonsResponse, priceOrderData } = state;
    const { userMessages = [] } = data?.data?.userMessages;

    const isOnlyActivation = finalData?.every(
        (row) => row.activityType === 'ACTIVATION'
    );

    const isOnlyAddLine = finalData?.every(
    ...
    getplansandaddonsResponse?.compatibility?.compatibilityInfo?.map((info) => {
        let newValue = finalData?.find(
            ({ telephoneData }) =>
                telephoneData &&
                telephoneData?.subscriberNumber === info?.subscriberNumber
        );
    ...
    priceOrderData?.linePriceDetails?.map((details) => {
        let compatibility = mappedPlansAndAddOnsData?.find(
            ({ ctn }) => details?.ctn === ctn
        );
    ...
    const isCreateOrderPopulated =
        Object.keys(createOrderData || {}).length > 0 &&
        Object.prototype.hasOwnProperty.call(createOrderData, 'uuid');
    ...
    const { charges, planDetails } = useMemo(
        () =>
            mapSourcesToStepComponents(
                priceOrderData,
                {
    ...
    useEffect(() => {
        if (isCreateOrderPopulated) {
            if (createOrderData.orderStepStatus === 'FAILURE') {
                const codes = [];
                const messages = [];
                createOrderData?.orderStepDetails?.forEach(
                    ({ code, message }) => {
                        codes.push(code);
                ...
                createOrderData?.lines?.forEach((lsd) =>
                    lsd?.lineStepDetails?.forEach(({ code, message }) => {
                        codes.push(code);
                        messages.push(message);
                    })
                );
                ...
                const errorCode =
                    codes.length > 1 ? `[${codes.join(',')}]` : codes[0];
                const errorMessage =
                    messages.length > 1
                        ? `[${messages.join(',')}]`
                        : messages[0];
                ...
                createOrderData.orderStepStatus === 'OKTOSUBMIT' ||
                createOrderData.orderStepStatus === 'INPROGRESS'
            ) {
                if (!(priceOrderData && priceOrderData.priceSummary)) return;

                const { dueAmount } = priceOrderData.priceSummary;
                const hasPortIn = createOrderData?.lines?.find(
                    ({ lineStep }) => lineStep === 'PORTIN'
                );
                ...
                if (hasDueAmount && effective === 'today') {
                    setState((v) => ({
                        ...
                        uiData: {
                            ...v.uiData,
                            hasDueAmount,
                            summarizedCharges: hasDueAmount
                                ? planDetails.summarizedCharges
                                : null,
                        },
                    }));
                } else if (hasPortIn) {
    ...
    useEffect(() => {
        if (
            lastAPICall === 'error/createOrder' ||
            lastAPICall === 'error/submitOrder'
        ) {
            setSubmitButtonData(() => ({
                isLoading: false,
                label:
    ...
    return (
        <div className="step-2-wrapper--parent">
            <ChargesCard
                {...props}
                charges={charges}
            ...
            <PlanList
                {...props}
                dueDate={moment(dueDate, 'YYYY-MM-DD').format('MM-DD-YYYY')}
                tableData={finalData}
