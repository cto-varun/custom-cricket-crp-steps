...
const AddALineForm = (props) => {
    ...
    const { validateAddLine, deviceInfo } = properties?.workflows;
    const {
        workflow,
        datasource,
        responseMapping,
        ...
        if (isSuccess || isFailure) {
            if (isSuccess) {
                ...
                if (
                    ...
                    const subscribers = [];
                    state?.tableData?.finalData?.forEach((td) => {
                        if (td.activityType === 'ADDLINE' || td.activityType === 'ACTIVATION') {
                            subscribers.push({
                                imei: td.telephoneData.imei,
                                socCode: td?.plan?.newPlan?.socCode,
                            });
                        } else if (td.activityType === 'CHANGESERVICES') {
                            subscribers.push({
                                subscriberNumber:
                                    td.telephoneData.subscriberNumber,
                                subscriberStatus: td.telephoneData.ptnStatus,
                                socCode: td?.plan?.currentPlan?.pricePlanSocCode,
                                imei: td?.telephoneData?.telephoneNumber,
                            });
                        }
                    });
            ...
            >
                <>
                    ...
                    {/* <Row gutter={24} style={{ textAlign: 'right' }}>
                        <Col span={6} new-passwordset={18}>
                            <Button
                                block
                                type="primary"
                                onClick={handleToggle}
                                className="addALine__button--cancel"
                            >
                                Cancel
                            </Button>
                        </Col>
