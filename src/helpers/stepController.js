...
export const sendSubmitOrderRequest = (state, requestBody = undefined) => {
    if (
        typeof window[window.sessionStorage?.tabId]
            .sendsubmitOrderAsyncMachine === 'function'
    ) {
        if (state.createOrderData) {
            const createOrderData = cloneDeep(state.createOrderData);
            if (requestBody) {
                window[
                    window.sessionStorage?.tabId
                ].sendsubmitOrderAsyncMachine('SET.REQUEST.DATA', {
                    value: requestBody,
                });
            }

            window[window.sessionStorage?.tabId].sendsubmitOrderAsyncMachine(
                'APPEND.URL',
                {
            ...
            setTimeout(
                () =>
                    window[
                        window.sessionStorage?.tabId
                    ].sendsubmitOrderAsyncMachine('FETCH'),
                500
            );
        }
    }
...
export const sendCreateOrderRequest = ({
    priceOrderData,
    applyCredit,
    tableData,
    ...
    if (
        typeof window[window.sessionStorage?.tabId]
            .sendcreateOrderAsyncMachine === 'function'
    ) {
        if (priceOrderData) {
            const priceOrderRequest = cloneDeep(statePriceOrderRequest);
            let {
                banStatus,
                accountType,
            ...
            const { linePriceDetails } = priceOrderData;

            const finalexistingLinesInfo = existingLinesInfo.map(
                (existingLine) => {
                    if (
                        linePriceDetails.find(
                            (line) => existingLine.ctn === line.ctn
                        ) !== undefined
                    ) {
                        const matchingLinePriceDetails = linePriceDetails.find(
                            (line) => existingLine.ctn === line.ctn
                        );

                        const existingLineInfo = {
                            ...existingLine,
                            ...(matchingLinePriceDetails.hasOwnProperty(
                            ...
                            ...(matchingLinePriceDetails.hasOwnProperty(
                                ...
                                currentAddOnPricingInfo:
                                    matchingLinePriceDetails.currentAddOnPricingInfo,
                            }),
                            ...(matchingLinePriceDetails.hasOwnProperty(
                            ...
                            ...(matchingLinePriceDetails.hasOwnProperty(
                                ...
                                nextBillAddOnPricingInfo:
                                    matchingLinePriceDetails.nextBillAddOnPricingInfo,
                            }),
                        };
                        ...
                        if (existingLineInfo?.currentPlanPricingInfo) {
                            transformEffectiveDates(
                                existingLineInfo.currentPlanPricingInfo
                                    ?.adjustmentEffectiveDate
                            );
                            transformEffectiveDates(
                                existingLineInfo.currentPlanPricingInfo
                                    ?.dueAmountEffectiveDate
                            );
                        }
                        if (existingLineInfo?.nextBillPlanPricingInfo) {
                            transformEffectiveDates(
                                existingLineInfo.nextBillPlanPricingInfo
                                    ?.dueAmountEffectiveDate
                            );
                        }
                        if (existingLineInfo?.nextBillAddOnPricingInfo) {
                            existingLineInfo.nextBillAddOnPricingInfo.forEach(
                                (addOnPricingInfo) => {
                                    transformEffectiveDates(
                                        addOnPricingInfo?.dueAmountEffectiveDate
                                    );
                                }
                            );
                        }
            ...
            const finalchangeOrNewLinesInfo = changeOrNewLinesInfo.map(
                (changeOrNewLine) => {
                    const matchingLinePriceDetails = linePriceDetails.find(
                        (line) =>
                            changeOrNewLine.status === 'RESERVED'
                                ? changeOrNewLine.lineIdentifier ===
                                  line.lineIdentifier
                                : changeOrNewLine.ctn === line.ctn
                    );
                    const matchingTableData = tableData?.finalData?.find((t) =>
                        changeOrNewLine.status === 'RESERVED'
                            ? t.telephoneData.imei ===
                              changeOrNewLine.newDeviceInfo.imei
                            : changeOrNewLine.ctn ===
                              t?.telephoneData?.telephoneNumber
                    );
                    if (matchingLinePriceDetails !== undefined) {
                        ...
                        if (matchingTableData) {
                            changedLineInfo.additionalInfo =
                                matchingTableData?.additionalInfo;
                        }

                        //  add Port information - This is new Line
                        if (changeOrNewLine.status === 'RESERVED') {
                            //  find matching tableInfo
                            const tabInfo = tableData.finalData.find(
                                (t) =>
                                    t.telephoneData.imei ===
                                    changeOrNewLine.newDeviceInfo.imei
                            );
                            if (
                                tabInfo.telephoneData.portInInfo !== undefined
                            ) {
                                changedLineInfo.portInInfo =
                                    tabInfo.telephoneData.portInInfo;
                            } else {
                                changedLineInfo.rateCenter =
                                    tabInfo.telephoneData.rateCenter;
                            }
                        }

                        if (changedLineInfo?.currentPlanPricingInfo) {
                            transformEffectiveDates(
                                changedLineInfo.currentPlanPricingInfo
                                    ?.adjustmentEffectiveDate
                            );
                            transformEffectiveDates(
                                changedLineInfo.currentPlanPricingInfo
                        ...
                        if (changedLineInfo?.currentAddOnPricingInfo) {
                            changedLineInfo.currentAddOnPricingInfo.forEach(
                                (addOnPricingInfo) => {
                                    transformEffectiveDates(
                                        addOnPricingInfo?.adjustmentEffectiveDate
                                    );
                                    transformEffectiveDates(
                                        addOnPricingInfo?.dueAmountEffectiveDate
                                    );
                        ...
                        if (changedLineInfo?.nextBillPlanPricingInfo) {
                            transformEffectiveDates(
                                changedLineInfo.nextBillPlanPricingInfo
                                    ?.adjustmentEffectiveDate
                            );
                            transformEffectiveDates(
                                changedLineInfo.nextBillPlanPricingInfo
                        ...
                        if (changedLineInfo?.nextBillAddOnPricingInfo) {
                            changedLineInfo.nextBillAddOnPricingInfo.forEach(
                                (addOnPricingInfo) => {
                                    transformEffectiveDates(
                                        addOnPricingInfo?.adjustmentEffectiveDate
                                    );
                        ...
                        // handle new accounts
                        if (
                            newAccountInfo?.additionalInfo &&
                            newAccountInfo?.imei &&
                            changedLineInfo?.newDeviceInfo?.imei?.toString() ===
                                newAccountInfo?.imei?.toString()
                        ) {
                            changedLineInfo.additionalInfo =
                                newAccountInfo?.additionalInfo;
                        }
            ...
            window[window.sessionStorage?.tabId].sendcreateOrderAsyncMachine(
                'RESET'
            );
            ...
            setTimeout(
                () =>
                    window[
                        window.sessionStorage?.tabId
                    ].sendcreateOrderAsyncMachine('REFETCH'),
                500
            );
        }
    }
...
const handleFirstStep = (state, setState, { stateFunction }) => {
    if (state.priceOrderData !== undefined) {
        if (
            state.priceOrderData?.linePriceDetails === undefined &&
            state.priceOrderData?.priceSummary === undefined
        ) {
            setState((v) => ({
                ...v,
                uiData: {
...
const handleSecondStep = (state, setState, { setSubmitButtonData }) => {
...
const handleThirdStep = (
    state,
    setState,
    { activeKey, paymentData, setSubmissionButtonData }
) => {
    if (paymentData) {
        if (
            paymentData.creditCardPaymentDetails?.confirmationCode ||
            paymentData.creditCardPaymentDetails?.transactionReferenceNumber ||
...
const processNext = (current, state, setState, options) => {
    if (current === 0) {
        ...
        setState((v) => ({ ...v, current: v.current + 1 }));
    } else if (current === 4) {
        handleThirdStep(state, setState, options);
    }
};

export const sendcrpPaymentFormValueCollector = (state) => (input) => {
...
export const reset = (setState) => () => {
    setState((v) => ({ ...v, current: 0 }));
};

export const next = (setState) => () => {
    setState((v) => ({ ...v, current: v.current + 1 }));
};

export const prev = (setState) => () => {
    setState((v) => ({ ...v, current: v.current > 0 ? v.current - 1 : 0 }));
};
...
const processBackStep3 = (state) => {
    ...
    setTimeout(
        () =>
            window[window.sessionStorage?.tabId].sendcancelCRPOrder('REFETCH'),
        500
    );
};

const stepController = (direction, current, [state, setState], options) => {
    if (direction === 'next') {
