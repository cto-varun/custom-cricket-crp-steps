...
const getAccountInfoObj = (
    ...
    let { autoPayIndicator, bridgePayIndicator } = accountInfo;
    let { accountSubType } = customerInfo;
...
function getUnchangedLines(tableInfo, plansAndAddons, ebbDetails) {
    ...
    const ebbEnrolled =
        ebbDetails && ebbDetails?.ebbEnrolledStatus === 'ENROLLED';
    tableInfo?.forEach((line) => {
        if (
            ...
            const compatibilityInfoForLine =
                compatibilityInfo[
            ...
            unchangedLines.push({
                ...
                ebbEnrolledIndicator:
                    ebbEnrolled &&
                    ebbDetails?.ebbEnrolledCtns?.find(
                        ({ ctn }) =>
                            ctn === line?.telephoneData?.telephoneNumber
                    )
                        ? true
                        : false,
                pricePlan:
                    compatibilityInfoForLine?.subscriberInfo?.currentRatePlan
                        .pricePlanSocCode,
                status: compatibilityInfoForLine?.ctnStatus,
                subscriberStatusReasonCode:
                    line?.telephoneData?.statusReasonCode,
                subscriberStatusDate:
                    compatibilityInfoForLine?.subscriberStatusDate,
                addOns: compatibilityInfoForLine?.subscriberInfo?.currentAddOns,
                latestCharge:
                    compatibilityInfoForLine?.subscriberInfo?.latestCharge,
            });
        }
    });
...
function getChangedLines(tableInfo, plansAndAddons, effectiveDate, ebbDetails) {
    ...
    tableInfo?.forEach((line) => {
        if (
            ...
            const hasValidCompatibilityInfo = (compatibilityInfoForLine || {})
                .subscriberInfo;
            ...
            if (Object.keys(line?.plan?.currentPlan || {}).length > 0) {
                pricePlanInfo.oldPricePlan =
                    line.plan.currentPlan.pricePlanSocCode;
            }
            ...
            if (isHasValidCompatibilityInfo) {
                compatibilityInfoForLine.subscriberInfo.currentAddOns.forEach(
                    (addon) => {
            ...
            line.changes?.forEach((change) => {
                ...
                const { addOnType = 'REGULAR' } =
                    line.addOns.find((a) => a.socCode === socCode) ||
                    line.technicalSocs.find((a) => a.socCode === socCode) ||
                    {};
                changedOrRemovedSoc.push(socCode);
                const { changeType } = change[socCode];
                const currentQuantity = mapOfAddons[socCode]?.quantity
                    ? mapOfAddons[socCode]?.quantity
                    : 0;
                switch (changeType) {
                    case 'removedAddOn':
                        if (
                            removeAddOnsInfo.findIndex(
                                (addOn) => addOn.socCode === socCode
                            ) === -1
                        ) {
                    ...
                    case 'newAddOn':
                        if (
                            newAddOnsInfo.findIndex(
                                (addOn) => addOn.socCode === socCode
                            ) === -1
                        ) {
                            newAddOnsInfo.push({
                                addOnCode: socCode,
                                addOnType, // This mapping info should be added to changes, right now hardcoded
                                quantity:
                                    change[socCode]?.quantity - currentQuantity, // Subtract current quantity (if it exists) from total quantity
                            });
                        }
                        break;
                    case 'addOnQuantity':
                        if (
                            ...
                            addOnIdx = newAddOnsInfo.findIndex(
                                (addOn) => addOn.socCode === socCode
                            );
                            newAddOnsInfo[addOnIdx].quantity =
                                change[socCode]?.quantity - currentQuantity;
                        }
                        break;
                    default:
                        break;
            ...
            line.technicalSocs?.forEach((addOn) => {
                ...
                const { addOnType = '' } =
                    line.technicalSocs.find((a) => a.socCode === socCode) || {};
                const changeType =
                    Object.keys(addOn?.changes).length > 0
                        ? Object.keys(addOn?.changes)[0]
                        : '';
                switch (changeType) {
                    case 'removedAddOn':
                        removeAddOnsInfo.push({
            ...
            if (isHasValidCompatibilityInfo) {
                compatibilityInfoForLine.subscriberInfo.currentAddOns?.forEach(
                    (addon) => {
                        if (
                            !(
                                changedOrRemovedSoc.includes(addon.addOnCode) ||
                                removeAddOnsInfo.findIndex(
                                    (removedAddOn) =>
                                        removedAddOn.addOnCode ===
                                        addon.addOnCode
                                ) > -1
                            )
                        ) {
            ...
            if (isHasValidCompatibilityInfo) {
                ...
                changedLineInfo.subscriberStatusReasonCode =
                    line?.telephoneData?.statusReasonCode;
                changedLineInfo.subscriberStatusDate =
                    compatibilityInfoForLine.subscriberStatusDate;
                changedLineInfo.latestCharge =
                    compatibilityInfoForLine.subscriberInfo.latestCharge;
                changedLineInfo.newDeviceInfo =
                    line.telephoneData.newDeviceInfo;
            } else {
...
export default function getPriceOrderRequest(
    tableInfo,
    plansAndAddons,
    applyCredit,
    effectiveDateString,
    ...
    const priceOrderRequest = {
        ...
        existingLinesInfo: getUnchangedLines(
            tableInfo,
            plansAndAddons,
            accountInfo?.ebbDetails
        ),
        changeOrNewLinesInfo: getChangedLines(
            tableInfo,
            plansAndAddons,
            effectiveDateString,
            accountInfo?.ebbDetails
