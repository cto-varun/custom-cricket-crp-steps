...
const createRuleset = (processor, compatibilityInfo) =>
    processor.load(compatibilityInfo).extractData({
        extract: [
            {
                name: 'data',
...
const createCurrentAddonsPerLine = (processor, compatibilityInfo) =>
    processor.load(compatibilityInfo).extractData({
        extract: [
            {
                name: 'currentAddons',
                ...
                transformations: [
                    (d) => {
                        return d.map((line) => {
                            ...
                            ] = line.subscriberInfo.currentAddOns.map(
                                (ca) => ca.addOnCode
                            );
...
const filterTabData = ({
    ...
    const filteredAccordionData = cloneDeep(tabData);
...
const getRowData = (tableData, rowIndex) => {
    ...
    if (tableData[rowIndex]) {
        data = cloneDeep(tableData[rowIndex]);
    }
    return data;
};
...
const transform2TableData = (plansAndAddons, subscriberInfo) => {
    ...
    subscriberInfo
        .filter((s) => s.ptnStatus === 'A' || s.ptnStatus === 'S')
        .forEach((s, i) => {
    ...
    plansAndAddons.compatibility.compatibilityInfo.forEach((s) => {
        ...
        if (tabInfoForSub !== undefined) {
            ...
            const currentRatePlanFeatures =
                plansAndAddons?.plans?.currentPlans
                    ?.find((plan) => plan?.socCode === currentPricePlanSoc)
        ...
        if (s.plans[currentPricePlanSoc]) {
            mandatoryAddons =
                s.plans[currentPricePlanSoc]?.mandatoryAddOns || [];
        }
            ...
            (currentAddon, i) => {
                ...
                if (!addonSocMeta.technical) {
                    ((tabInfoForSub || {}).addOns || [])[i] = {
                        ...
                        mandatory: mandatoryAddons.includes(
                            currentAddon?.addOnCode
                        ),
                        changes: {},
                    };
                } else {
...
const processTable = (
    ...
    [apiData, tab],
    [state, setState],
    [compatibilityMessage, setCompatibilityMessage],
    queriesData
) => {
    const {
    ...
    const { lineDetails } = state;

    const ruleset = createRuleset(processor, compatibilityResponseObject);
    const currentAddonList = createCurrentAddonsPerLine(
        processor,
    ...
    if (lineDetails) {
        const initialTelephoneData = cloneDeep(lineDetails);
    ...
    if (
        compatibilityResponseObject &&
        compatibilityResponseObject.compatibilityInfo
    ) {
    ...
    const getCompatibilityObjectForLine = (subscriberNumber) => {
        ...
        if (Object.keys(compatibilityResponseObject).length > 0) {
            const compatibilityLinesArray =
                compatibilityResponseObject.compatibilityInfo;
            compatibilityLinesArray.map((line) => {
                if (
                    line.subscriberNumber.toString() ===
    ...
    const {
        compatibility: { allowAddLine = true },
    } = tab;
